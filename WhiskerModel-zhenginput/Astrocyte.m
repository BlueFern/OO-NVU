classdef Astrocyte < handle
    ...
    methods
        ...
        function [du, varargout] = rhs(self, t, u, J_KIR_i, R, J_VOCC_i, NO_n, NO_i, J_K_NEtoSC, J_Na_NEtoSC, Glu)
            ...
            % Input of K+ and Na+ to the SC (assuming that the SC is a small 
            %part of the ECS and everything that happens to the ECS also happens to the SC)
            J_K_NEtoSC_k = J_K_NEtoSC * 1e3;                                % Convert from mM/ms to uM/ms
            J_Na_NEtoSC_k = J_Na_NEtoSC * 1e3;                              % Convert from mM/ms to uM/ms
...
%             J_TRPV_k = - I_TRPV_k / (p.z_Ca * p.C_astr_k * p.gamma_i); % TRPV4 flux [uM/s]
            ...
            minf_k = (1 ./ (1 + exp(-(eta - p.epshalf_k) ./ p.kappa_k))) .*...
                ((1 ./ (1 + H_Ca_k)) .* (H_Ca_k + tanh((v_k - p.v1_TRPV_k) ./ p.v2_TRPV_k))); 
...
function params = parse_inputs(varargin)
    ...
    parser.addParameter('G_BK_k', 10.25e-3)        % g_BK_k = 225 * 1e-12 / 3.7e-9 = 6.08e-2;
    parser.addParameter('G_K_k', 6907.77e-3);      % 40
    parser.addParameter('G_Na_k', 226.94e-3);      % 1.314
    parser.addParameter('G_NBC_k', 130.74e-3);     % 7.57e-1
    parser.addParameter('G_KCC1_k', 1.728e-3);     % 1e-2
    parser.addParameter('G_NKCC1_k', 9.568e-3);    % 5.54e-2
    parser.addParameter('G_Cl_k', 151.93e-3);      % 8.797e-1

    parser.addParameter('J_NaK_max', 2.3667e1);                             % [uM ms^-1]      1.42e-3
    ...
    parser.addParameter('k_on', 2e-3);                                      % [uM ms^-1]
    parser.addParameter('K_inh', 0.1); %uM
    parser.addParameter('r_h', 4.8e-3);                                     % [uM/ms]
    parser.addParameter('k_deg', 1.25e-3);                                  % [ms^-1]
    parser.addParameter('V_eet', 72e-3);                                    % [ms^-1]
    parser.addParameter('k_eet', 7.2e-3);                                   % [ms^-1]
    ...
    parser.addParameter('t_TRPV_k', 0.9e3);                                 % [ms] 
    parser.addParameter('R_init', 20); 
    parser.addParameter('Ca_decay_k', 0.5e-3);                              % [ms^-1]
    parser.addParameter('G_TRPV_k', 3.15e-7);                               % [ms^-1]
    parser.addParameter('r_buff', 0.05); % Rate at which Ca2+ from the TRPV4 channel 
    %at the endfoot is buffered compared to rest of channels on the astrocyte body [-]
    
    % Perivascular space
    parser.addParameter('VR_pa', 0.001);% [-]
    parser.addParameter('VR_ps', 0.001);% [-]
    parser.addParameter('R_decay', 0.15e-3);                                % [ms^-1]
...
%     parser.addParameter('F', 9.65e4); %C mol^-1; Faraday's constant
    ...
    parser.addParameter('J_max', 2880e-3);                                  % [uM ms^-1]
    parser.addParameter('K_act', 0.17); %uM
    parser.addParameter('P_L', 0.0804e-3);                                  % [uM ms^-1]
    parser.addParameter('V_max', 20e-3);                                    % [uM ms^-1]
    ...
    parser.addParameter('psi_w', 2.664e-3);                                 % [ms^-1]
    ...
    parser.addParameter('k_O2_k', 9.6e-9);                                  % [uM^-2 ms^-1] ;  (Kavdia2002)
