...
\subsection{The Extracellular Space Model (\textbf{new})}

Conservation equation of $K^+$ in the EC ($\mu$Ms$^{-1}$):
\begin{equation}
\frac{d K_e}{dt} = J_{K_i} - J_{NaK_i} - J_{diff}
\end{equation}

Diffusive flux of $K^+$ from the ECS into the SC ($\mu$Ms$^{-1}$):
\begin{equation}
J_{diff} = \frac{1}{\tau_s} \left( K_e - \frac{N_{K_s}}{R_s} \right)
\end{equation}

$J_{K_i}$: equation \ref{eq:JKi}.
$J_{NaK_i}$: equation \ref{eq:J_NaK_i}.

\begin{table}[h!]
\centering
\begin{tabular}{ p{0.09\linewidth}  >{\footnotesize} p{0.5\linewidth}  >{\footnotesize} p{0.27\linewidth} >{\footnotesize} p{0.03\linewidth} }
\hline
$\tau_s$ 		& Characteristic diffusion time between the SC and ECS 		& 2.8 s 		& ME 
\\
\hline
\end{tabular}
\end{table}
...
\begin{tabular}{ p{0.09\linewidth}  >{\footnotesize} p{0.5\linewidth}  >{\footnotesize} p{0.27\linewidth} >{\footnotesize} p{0.03\linewidth} }
\hline
$F_{\text{input}}$ 		& amplitude scaling factor 		& 2.67 		& ME\footnotemark[1]  \\ % on the basis of Filosa2008
...
\dfrac{\mathrm{d}N_{\text{K},s}}{\mathrm{d}t}= k_C f(t) -\dfrac{\mathrm{d}N_{\text{K},k}}{\mathrm{d}t} - J_{\text{BK},k}\ + R_s J_{diff},; \text{~~~} [\K]_s = \frac{N_{\text{K},s}}{R_s}
\end{equation}
%
