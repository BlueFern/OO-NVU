...
\dfrac{\mathrm{d}K_{p}}{\mathrm{d}t}= \frac{J_{BK_k}}{R_k R_{pa}} + \frac{J_{KIR_i}}{R_{ps}} - R_{decay}(K_p - K_{p,min});
\end{equation}
%
\begin{table}[h!]
\centering
...
$ R_{decay} $  & Decay rate & 0.05 s$^{-1}$ & M.E. \\
$ K_{p,min} $  & min K+ concentration & 3 $\times$ 10$^{3}$ \textmu M & M.E. \\
\hline
\end{tabular}
\end{table}
...
$G_{BK_k}$ 		& Specific ion conductance of the BK channel							& 4.3$\times$10$^3$   pS & \cite{GonzalezFernandez1994}  \\
$A_{ef,k}$		& Membrane area														    & 3.7 $\times$ 10$^{-9}$ & \cite{GonzalezFernandez1994} \\
$g_{BK_k}$ 		& Specific ion conductance of the BK channel							&  $G_{BK_k}$ $\times$10$^{-12}$/A$_{ef,k}$ = 1.16$\times$10$^3$ \perOhmm & \cite{GonzalezFernandez1994}  \\
...
$C_{unit}$ & Unit converting factor   & 10$^{3}$ & M.E. \\
\hline
\end{tabular}
\end{table}
...
v_k=\frac{g_{Na_k}E_{Na_k}+g_{K_k}E_{K_k}+g_{Cl_k}E_{Cl_k}+g_{NBC_k}E_{NBC_k} + g_{BK_k}w_kE_{BK_k} -J_{NaK_k}F C_{unit} }{ g_{Na_k}+g_{K_k}+g_{Cl_k}+g_{NBC_k}+g_{BK_k}w_k }
\end{equation}
%
