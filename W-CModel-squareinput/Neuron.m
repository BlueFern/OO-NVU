classdef Neuron < handle
    ...
    %% Algebraic Expressions
            % E and I cells
            k_e = self.s_e(10);                 % [-]                       Max value for response function E
            k_i = self.s_i(10);                 % [-]                       Max value for response function I
            arg_e = p.c1 .* E_t - p.c2 .* I_t + self.P(t);  % [-]           Excitatory cells response function input
            arg_i = p.c3 .* E_t - p.c4 .* I_t + self.Q(t);  % [-]           Inhibitory cells response function input
...
%             J_O2_pump       = p.CBF_init .* P_02 .* CMRO2_delay .* p.gamma_O2 .* ((J_pump1_sa ...
%                 + J_pump1_d) ./ (J_pump1init_sa + J_pump1init_d));
                                                % [mM s^-1]                 Total oxygen consumption of the NaK pumps
            J_O2_pump = self.J_O2pump(t, u);
            ...
            dKedt = -p.beta_K_e * (K_e - p.K_eBase) + ...
                (p.alpha_K_e .* p.beta_K_e) .* ((E_t - I_t) ./ p.EI_relative);
            J_K_NEtoSC = p.k_syn * dKedt;                                 
            J_Na_NEtoSC = -p.k_syn * dKedt;
            
      %% Derivative equations                                                                   
            % Excitatory and inhibitory cells (Input of this model)
            du(idx.E_t, :) = 1 ./ p.tau_e .* (-E_t + (k_e - p.r_e .* E_t)...
                .* self.s_e(arg_e));            % [ms^-1]                   Change in excitatory cells firing
            du(idx.I_t, :) = 1 ./ p.tau_i .* (-I_t + (k_i - p.r_i .* I_t)...
                .* self.s_i(arg_i));            % [ms^-1]                   Change in inhibitory cells firing
            ...
            du(idx.Na_sa, :) = - p.beta_Na_sa * (Na_sa - p.Na_saBase) + ...
                p.alpha_Na_sa * p.beta_Na_sa .* ((E_t - I_t) ./ p.EI_relative);
                                                % [mM ms^-1]                Change in soma/axon sodium concentration
            du(idx.Na_d, :) = - p.beta_Na_d * (Na_d - p.Na_dBase) + ...    
                p.alpha_Na_d * p.beta_Na_d .* ((E_t - I_t) ./ p.EI_relative);
            ...
            if nargout == 2
                ...
                Uout(self.idx_out.J_O2pump, :) = self.J_O2pump(t,u);
            	varargout = {Uout};
            end
        end
        ...
        function [Glu, J_K_NEtoSC, J_Na_NEtoSC, NO_n, O2] = shared(self, ~, u)
            ...
            dKedt = -p.beta_K_e * (K_e - p.K_eBase) + (p.alpha_K_e ...
                 .* p.beta_K_e) .* ((E_t - I_t) ./ p.EI_relative);          % K_e derivative as neuron activation in Ostby
            J_K_NEtoSC = p.k_syn * dKedt;                                   % Input flux: K+ flux into SC
            J_Na_NEtoSC = -p.k_syn * dKedt;                                 % Input flux: Na+ flux out of SC
        end       
        
        function sigmoid_e = s_e(self,x)                                    % Response function for excitatory neurons
            p = self.params;
            sigmoid_e = 1 ./ (1 + exp(-(p.a_e .* (x - p.theta_e)))) - 1 ./ (1 + exp(p.a_e .* p.theta_e));
        end
        function sigmoid_i = s_i(self,x)                                    % Response function for inhibitory neurons 
            p = self.params;
            sigmoid_i = 1 ./ (1 + exp(-(p.a_i .* (x - p.theta_i)))) - 1 ./ (1 + exp(p.a_i .* p.theta_i));
        end
        function input_e = P(self,t)                                        % External input given to excitatory cells
            p = self.params;
            if p.Whiskerpad == 0
            ...
            elseif p.Whiskerpad == 1 || p.Whiskerpad == 2 || p.Whiskerpad == 4
                ...
                input_e = p.Pinput .* neural_data(index_t);
            elseif p.Whiskerpad == 3
                pulse_width_P = p.startpulse + p.lengthpulse;
                pulse_width_P_1 = p.secondpulse + p.secondlength;
                input_e = p.Pinput .* (heaviside(t - p.startpulse) - ...
                    heaviside(t - pulse_width_P) + heaviside(t - p.secondpulse)...
                    - heaviside(t - pulse_width_P_1));
            end
        end
        function input_i = Q(self,t)                                        % External input given to inhibitory cells
            p = self.params;
            if p.Whiskerpad == 0
                pulse_width_P = p.startpulse + p.lengthpulse;
            ...
            elseif p.Whiskerpad == 1 || p.Whiskerpad == 2 || p.Whiskerpad == 4
                ...
                neural_data = self.input_data;
                index_t = round(t/dt + 1);
                input_i = p.Qinput .* neural_data(index_t);
            elseif p.Whiskerpad == 3
                pulse_width_P = p.startpulse + p.lengthpulse;
                pulse_width_P_1 = p.secondpulse + p.secondlength;
                input_i = p.Qinput .* (heaviside(t - p.startpulse) - ...
                    heaviside(t - pulse_width_P) + heaviside(t - p.secondpulse)...
                    - heaviside(t - pulse_width_P_1));
            end
        end
        
        function J_O2pump = J_O2pump(self,t,u)
            p = self.params;
            O2 = u(self.index.O2, :);
            K_e = u(self.index.K_e, :);
            Na_sa = u(self.index.Na_sa, :);
            Na_d = u(self.index.Na_d, :);
            ...
            J_pump1init_sa  = 0.0312;                      
            J_pump1_d       = (1 + (p.K_init_e ./ K_e)).^(-2) .* (1 + (p.Na_init_d ./ Na_d)).^(-3);   
            J_pump1init_d   = 0.0312;           
            
            minJO2pump =  4.012e-3;  % Min value for J_O2pump
            maxJO2pump = 7.8047e-3; % Max value for J_O2pump, depends on length of stimulation. 2 sec: 4.8045e-3, 8 sec: 7.7223e-3, 16 sec: 7.8047e-3
            
            % Delay the increase in J_O2pump and hence delay CMRO2 and make the initial dip smaller, with t params in ms
            delay1 = 5; % sec to delay increase
            delay2 = 5; % sec to delay decrease
            CMRO2_delay_up = 0.5 + 0.5*tanh((t-(p.startpulse+500*delay1))/(200*delay1));
            CMRO2_delay_down = 0.5*tanh((t-((p.startpulse+p.lengthpulse)))/(0.001)) - 0.5*tanh((t - (p.startpulse+p.lengthpulse+500*delay2))/(200*delay2));

            f_O2pump = p.CBF_init .* P_02 .* p.gamma_O2 .* ((J_pump1_sa + J_pump1_d) ./ (J_pump1init_sa + J_pump1init_d));
            
%             J_O2pump = minJO2pump + (f_O2pump - minJO2pump).*CMRO2_delay_up; % Only delay start
%             J_O2pump = f_O2pump + (maxJO2pump-f_O2pump).*CMRO2_delay_down; % Only delay end
% 
            J_O2pump = minJO2pump + (f_O2pump - minJO2pump).*CMRO2_delay_up + (maxJO2pump-f_O2pump).*CMRO2_delay_down; % Delay start and end
...
function params = parse_inputs(varargin)
    ...
    parser.addParameter('K_eBase', 3.5);        % [mM]
    parser.addParameter('Na_saBase', 9.37);     % [mM]
    parser.addParameter('Na_dBase', 9.42);      % [mM]
    
    parser.addParameter('alpha_K_e', 3.2);      % [-]
    parser.addParameter('beta_K_e', 4.2e-3);    % [ms^-1]
    parser.addParameter('alpha_Na_sa', 4.23);   % [-]                     
    parser.addParameter('beta_Na_sa', 0.39e-3); % [ms^-1]
    parser.addParameter('alpha_Na_d', -2.12);   % [-]                    
    parser.addParameter('beta_Na_d', 0.75e-3);  % [ms^-1]
    
    % E and I parameters
    parser.addParameter('EI_relative', 0.2894); % [-]                       Maximum value for E-I after external inputs
    parser.addParameter('c1', 12);              % [-]
    parser.addParameter('c2', 10);              % [-]
    parser.addParameter('c3', 13);              % [-]
    parser.addParameter('c4', 11);              % [-]
    parser.addParameter('r_e', 1.0);            % [ms]?
    parser.addParameter('r_i', 4.0);            % [ms]?
    parser.addParameter('tau_e', 3);            % [ms]
    parser.addParameter('tau_i', 3);            % [ms]
    
    parser.addParameter('a_e', 1.2);            % [-]
    parser.addParameter('theta_e', 2.8);        % [-]
    parser.addParameter('a_i', 1.0);            % [-]
    parser.addParameter('theta_i', 4.0);        % [-]
