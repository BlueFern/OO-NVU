classdef SMCEC < handle
    ...
    methods
        ...
        function [J_KIR_i, Ca_i, J_VOCC_i, NO_i, R_cGMP2] = shared(self, t, u, K_p)
            ...
            G_KIR_i = p.F_KIR_i * exp(p.z_5 * v_i + p.z_3 * K_p);           % Changed by including exp(-p.z_4) parameter into
            % G_KIR_i = p.F_KIR_i * exp(p.z_5 * v_i + p.z_3 * K_p - p.z_4); % F_KIR_i parameter, no large constant in exponential equation
...
function params = parse_inputs(varargin)
    ...
    parser.addParameter('lambda_i', 45e-3);                                 % [ms^-1]

    % Endothelial Cell ODE Constants
    parser.addParameter('C_m_j', 25.8);                                     % pF veranderen naar iets met ms??
    parser.addParameter('J_PLC', 0.11e-3);%0.11 for steady state, 0.3 for oscillations % [uM ms^-1] 
    parser.addParameter('J_0_j', 0.029e-3);                                 % [uM ms^-1] constant Ca influx (EC)

    % Smooth Muscle Cell Flux Constants
    parser.addParameter('F_i', 0.23e-3);                                    % [uM ms^-1]      % IP3/RYR channel strength
    parser.addParameter('K_r_i', 1); %uM

    parser.addParameter('B_i', 2.025e-3);                                   % [uM ms^-1]     % SERCA pump strength     
    parser.addParameter('c_b_i', 1.0); %uM

    parser.addParameter('C_i', 55e-3);                                      % [uM ms^-1]
    parser.addParameter('s_c_i', 2.0); %uM
    parser.addParameter('c_c_i', 0.9); %uM

    parser.addParameter('D_i', 0.24e-3);                                    % [ms^-1]
    parser.addParameter('v_d', -100); %mV
    parser.addParameter('R_d_i', 250); %mV

    parser.addParameter('L_i', 0.025e-3);                                   % [ms^-1]

    parser.addParameter('G_Ca_i', 1.29e-6);                                 % [uM mV^-1 ms^-1]
    parser.addParameter('v_Ca1_i', 100); %mV
    parser.addParameter('v_Ca2_i', -24); %mV
    parser.addParameter('R_Ca_i', 8.5); %mV

    parser.addParameter('G_NaCa_i', 3.16e-6);                               % [uM mV^-1 ms^-1]
    parser.addParameter('c_NaCa_i', 0.5); %uM
    parser.addParameter('v_NaCa_i', -30); %mV

    parser.addParameter('G_stretch', 6.1e-6);                               % [uM mV^-1 ms^-1]   (Also EC parameter)
    ...
    parser.addParameter('F_NaK_i', 4.32e-5);                                % [uM ms^-1]

    parser.addParameter('G_Cl_i', 1.34e-6);                                 % [uM mV^-1 ms^-1]
    parser.addParameter('v_Cl_i', -25); %mV

    parser.addParameter('G_K_i', 4.46e-6);                                  % [uM mV^-1 ms^-1]
    parser.addParameter('v_K_i', -94); %mV

    parser.addParameter('F_KIR_i', 1.285e-9);%0.381);                       % [uM mV^-1 ms^-1] F_KIR_i = 1.285e-6
    parser.addParameter('k_d_i', 0.1e-3);                                   % [ms^-1]

    % Endothelial Cell Flux Constants
    parser.addParameter('F_j', 0.23e-3);                                    % [uM ms^-1]
    parser.addParameter('K_r_j', 1); %uM
    parser.addParameter('B_j', 0.5e-3);                                     % [uM ms^-1]
    parser.addParameter('c_b_j', 1); %uM
    parser.addParameter('C_j', 5e-3);                                       % [uM ms^-1]
    parser.addParameter('s_c_j', 2); %uM
    parser.addParameter('c_c_j', 0.9); %uM
    parser.addParameter('D_j', 0.24e-3);                                    % [ms^-1]

    % (G_stretch, alpha_stretch, trans_p_mmHg, sigma0, E_SAC are included above in 
    %  SMC flux Constants)

    parser.addParameter('L_j', 0.025e-3);                                   % [ms^-1] 

    parser.addParameter('G_cat_j', 6.6e-7);                                 % [uM mV^-1 ms^-1]
    parser.addParameter('E_Ca_j', 50); %mV
    parser.addParameter('m_3_cat_j', -0.18); 
    parser.addParameter('m_4_cat_j', 0.37); 

    parser.addParameter('G_tot_j', 6927e9);                                 % [mpS] = [kg^-1*m^-2*ms^3*A^2] --> WRONG?
    ...
    parser.addParameter('G_R_j', 955e9);                                    % [mpS] = [kg^-1*m^-2*ms^3*A^2] --> WRONG?
    parser.addParameter('v_rest_j', -31.1); %mV
    parser.addParameter('k_d_j', 0.1e-3);                                   % [ms^-1]

    parser.addParameter('P_Ca', 0.05e-3);                                   % [ms^-1]
    parser.addParameter('P_IP3', 0.05e-3);                                  % [ms^-1]
    parser.addParameter('G_coup', 0.5e-3);                                  % [ms^-1]
    ...
    % NO pathway
    parser.addParameter('D_cNO', 3300e-3);                                  % [um^2 ms^-1] ; 
    %Diffusion coefficient NO (Malinski1993)
    parser.addParameter('K_mArg_j', 1.5); % [uM] ;
    parser.addParameter('K_mO2_j', 7.7); % [uM] ; Chen2006
    parser.addParameter('k_dno', 0.01e-3);                                  % [ms^-1] ;
    parser.addParameter('K_m_mlcp', 5.5); % [uM] ;
    parser.addParameter('V_NOj_max', 1.22e-3);                              % [ms^-1] ; 
    %maximum catalytic rate of NO production (Chen2006) - obtained from fig 6 & equ 17 & 18
    parser.addParameter('LArg_j', 100); % [uM] ;
    parser.addParameter('k_O2', 9.6e-9);                                    % [uM^-2 ms^-1] ;
    ...
    parser.addParameter('k_1', 100e-3);                                     % [ms^{-1}] ;
    parser.addParameter('k1', 2);                                           % [uM^-1 ms^-1] ;
    parser.addParameter('k2', 0.1e-3);                                      % [ms^-1] ;
    parser.addParameter('k3', 3e-3);                                        % [uM^-1 ms^-1] ;
    parser.addParameter('V_max_sGC', 0.8520e-3);                            % [uM/ms] ;
    parser.addParameter('k_pde', 0.0195e-3);                                % [ms^-1] ;
    parser.addParameter('C_4', 0.011e-3);                                   % [ms^{-1} microM^{-2}] ;
    parser.addParameter('K_m_pde', 2); % [uM] ;
    parser.addParameter('gam_eNOS', 0.1); % [-] ;
    parser.addParameter('mu2_j', 0.0167e-3);                                % [ms^-1] ;
    parser.addParameter('K_dis', 9e-5);                                     % [uM ms^-1] ;
    parser.addParameter('K_eNOS', 4.5e-1); % [uM] ;    
    parser.addParameter('g_max', 0.06e-3);                                  % [uM ms^-1] ;    
