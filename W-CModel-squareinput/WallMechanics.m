...
function params = parse_inputs(varargin)                                    
    ...
    parser.addParameter('K_3', 0.4e-3);                                     % [ms^-1]
    parser.addParameter('K_4', 0.1e-3);                                     % [ms^-1]
    parser.addParameter('K_7', 0.1e-3);                                     % [ms^-1]
    parser.addParameter('gamma_cross', 17e-3);                              % [uM^-3 ms^-1]
    parser.addParameter('n_cross', 3);                                      % [-]
    
    % Mechanical Equation Constants
    parser.addParameter('eta', 1e7);                                        % [Pa ms]
    parser.addParameter('R_init', 20);                                      % [um]
    parser.addParameter('trans_p', 4000);                                   % [Pa]
    parser.addParameter('E_passive', 66e3);                                 % [Pa]
    parser.addParameter('E_active', 233e3);                                 % [Pa]
    parser.addParameter('alpha', 0.6);                                      % [-]
    parser.addParameter('k_mlcp_b', 0.0086e-3);                             % [ms^-1]
    parser.addParameter('k_mlcp_c', 0.0327e-3);                             % [ms^-1]
...
function u0 = initial_conditions(idx)                                       
    ...
    % Initial estimations of parameters from experimental data
    u0(idx.Mp) = 0.0842;                                                    % [-]
    u0(idx.AMp) = 0.0622;                                                   % [-]
    u0(idx.AM) = 0.2746;                                                    % [-]
    u0(idx.R) = 22.97;                                                      % [um]
