...
defaultanswer_1 = {'8','4','0.8','0.005','0.1','y','5','15','0.02','3','0.55','740'};                 % Just a default answer
...
Results_end_max(ga)  = max(time_results_c_k(timestep,2:j+1)');                     %essential for plotting results
Results_end_mean(ga) = mean(time_results_c_k(timestep,2:j+1)');                    %essential for plotting results 
...
% decision
sel_output      = zeros(nr_parents,4+nr_par);                                  %creates a zero matrix 4 because of probabilities, numberation
...
var_p = 1;
for pp = 1:sel_2/2                                                          %divided by 2, because there are 2 parents   
   ...
   for mu_2 = 1 : nr_par                                                    % Running variable for number of parameters / columns
       ...
       if r_mu_1 <= p_mu_ini                                                % decision of mutation occurs or not 
           ...
           if mu_2 == 1;                                                    %defines parameter specific boundaries
                r_mu_2 = low_b_k_deg + (up_b_k_deg - low_b_k_deg)*rand(1,1);      
                parent_matrix(mu_1,mu_2 + 4) = r_mu_2;                                 
           elseif mu_2 == 2;
                r_mu_2 = low_b_k_on + (up_b_k_on - low_b_k_on)*rand(1,1);      
                parent_matrix(mu_1,mu_2 + 4) = r_mu_2;                                 
           elseif mu_2 == 3;
                r_mu_2 = low_b_k_inh + (up_b_k_inh - low_b_k_inh)*rand(1,1);      
                parent_matrix(mu_1,mu_2 + 4) = r_mu_2;                                 
           elseif mu_2 == 4;
                r_mu_2 = low_b_r_h + (up_b_r_h - low_b_r_h)*rand(1,1);      
                parent_matrix(mu_1,mu_2 + 4) = r_mu_2;                                 
           elseif mu_2 == 5
                r_mu_2 = low_b_K_G + (up_b_K_G - low_b_K_G)*rand(1,1);      
                parent_matrix(mu_1,mu_2 + 4) = r_mu_2;                                 
           elseif mu_2 == 6;
                r_mu_2 = low_b_K_I + (up_b_K_I - low_b_K_I)*rand(1,1);      
                parent_matrix(mu_1,mu_2 + 4) = r_mu_2;                                 
           elseif mu_2 == 7;
                r_mu_2 = low_b_BK_end + (up_b_BK_end - low_b_BK_end)*rand(1,1);      
                parent_matrix(mu_1,mu_2 + 4) = r_mu_2;                                 
           elseif mu_2 == 8;
                r_mu_2 = low_b_K_ex + (up_b_K_ex - low_b_K_ex)*rand(1,1);      
                parent_matrix(mu_1,mu_2 + 4) = r_mu_2;                                 
           elseif mu_2 == 9;
                r_mu_2 = low_b_B_ex + (up_b_B_ex - low_b_B_ex)*rand(1,1);      
                parent_matrix(mu_1,mu_2 + 4) = r_mu_2;                                 
           elseif mu_2 == 10;
               r_mu_2 = low_b_J_max + (up_b_J_max - low_b_J_max)*rand(1,1);      
                parent_matrix(mu_1,mu_2 + 4) = r_mu_2;                                 
           elseif mu_2 == 11;
               r_mu_2 = low_b_K_act + (up_b_K_act - low_b_K_act)*rand(1,1);      
                parent_matrix(mu_1,mu_2 + 4) = r_mu_2;                                 
           elseif mu_2 == 12;
               r_mu_2 = low_b_P_L + (up_b_P_L - low_b_P_L)*rand(1,1);      
                parent_matrix(mu_1,mu_2 + 4) = r_mu_2;                                 
           elseif mu_2 == 13;
               r_mu_2 = low_b_V_max + (up_b_V_max - low_b_V_max)*rand(1,1);      
                parent_matrix(mu_1,mu_2 + 4) = r_mu_2;                                 
           elseif mu_2 == 14;
               r_mu_2 = low_b_k_pump + (up_b_k_pump - low_b_k_pump)*rand(1,1);      
                parent_matrix(mu_1,mu_2 + 4) = r_mu_2;                                 
           elseif mu_2 == 15;
               r_mu_2 = low_b_delta + (up_b_delta - low_b_delta)*rand(1,1);      
                parent_matrix(mu_1,mu_2 + 4) = r_mu_2;                                 
           elseif mu_2 == 16;
               r_mu_2 = low_b_VR_ER_cyt + (up_b_VR_ER_cyt - low_b_VR_ER_cyt)*rand(1,1);      
                parent_matrix(mu_1,mu_2 + 4) = r_mu_2;                                             
           end
           end
...
parent_matrix(g,4)          =  results_c_k_gen(g,timestep);
end

Results_end_max(ga+1)  = max(results_c_k_gen(:,timestep)');                        %essential for plotting results
Results_end_mean(ga+1) = mean(results_c_k_gen(:,timestep)');                       %This is located afeter each generation to save the CA result
...
for mu_1 = 1:length(parent_matrix(:,1))
   ...
   for mu_2 = 1 : nr_par
       ...
       if mu_current >= mu_mean                                                     %makes mutation rate adaptive to quality of organism's fitness
            p_mu(mu_var) = (p_mu_ini / 2) * ((mu_max - mu_current) / (mu_max - mu_mean));     %If organism is fitter than average -> mutation should decrease
       else 
            p_mu(mu_var) = (p_mu_ini / 2);                                            %if not mutation rate stays the same
       end
