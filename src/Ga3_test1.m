...
us_prompt_param = { 'How many parameter?'
    'Parameter1:'
    'Parameter2:'
    'Parameter3:'
    'Parameter4:'
...
us_title_param = 'Which parameters do you want to use?';

us_lines_param = 1;

us_defaultanswer_param = {'16','k_deg','k_on','K_inh','r_h','K_G','K_I','BK_end','K_ex','B_ex','J_max','K_act','P_L','V_max','k_pump','delta','VR_ER_cyt','0','0','0','0'};                 % Just a default answer

us_param = inputdlg(us_prompt_param,us_title_param,us_lines_param,us_defaultanswer_param);

ini_NrPar = str2double(us_param{1});

for us_i=1:ini_NrPar
    us_a = nv.astrocyte.params;
    ini_value(us_i) = getfield(us_a, us_param{us_i+1});
end
...
us_defaultanswer_1 = {'8','4','0.8','0.005','0.1','n','5','15','0.02','3','0.55','740','0.02'};                 % Just a default answer
...
if check == 1
    ...
    for ini_prompt = 1 : ini_NrPar
        A = {'Enter lower boundary of randomisation of', us_param{ini_prompt+1}};
        B = {'Enter upper boundary of randomisation of', us_param{ini_prompt+1}};
    ...
    ini_prompt_1 = ini_prompt_vec(1:ini_NrPar);
    x = inputdlg(ini_prompt_1,dlg_title_2,num_lines_2,defaultanswer_2,'on');
    ...
    x_3 = inputdlg(ini_prompt_3,dlg_title_3,num_lines_3,defaultanswer_3,'on');
    
    for ini_i = 1 : ini_NrPar/2
        
        low_b(ini_NrPar/2 + ini_i)           = str2double(x_3{ini_i * 2 -1});
        up_b(ini_NrPar/2 + ini_i)            = str2double(x_3{ini_i * 2});
        low_b(ini_i)                         = str2double(x{ini_i * 2 -1});
        up_b(ini_i)                          = str2double(x{ini_i * 2});
    end
    ini_i = 1;
    ...
    for ini_i = 1 : ini_NrPar
        ...
        low_b(ini_i)           = str2double(x_4{1});
        up_b (ini_i)           = str2double(x_4{2});
...
for ini_k = 1:ini_popsize
    ...
    for ini_i = 1:ini_NrPar
        nv.astrocyte.params.(us_param{ini_i+1}) = ini_value(ini_i) * ini_randMat(ini_i,ini_k);
    end
    nv.simulate()
    
    results_c_k (ini_k,:)= nv.out('c_k');
    
end

Max_matrix = [results_c_k(:,timestep),ini_randMat'];                     %takes c_k value at time 740ms because this is the area of interest and combines it with the responsible param. factors

Results_end_max(r_simu_2)  = max(results_c_k(:,timestep));                     %essential for plotting results
Results_end_mean(r_simu_2) = mean(results_c_k(:,timestep));                    %essential for plotting results
...
%probabilities
sel_sum_c_k = sum(Max_matrix(1:ini_popsize,1));                                         %sum of all calcium concentrations of the generation
...
for mpar_i = 1:sel_2/2                                                          %divided by 2, because there are 2 parents
    ...
    for mpar_j = 1:(ini_popsize/nr_parents)                                                         % Repeat until initial population size is reached
...
for ini_k = 1:ini_popsize
    ...
    for ini_i = 1:ini_NrPar
        nv.astrocyte.params.(us_param{ini_i+1}) = ini_value(ini_i) * parent_matrix(ini_k,ini_i+4);
    end
    nv.simulate()
    
    results_c_k (ini_k,:)= nv.out('c_k');
    parent_matrix(ini_k,4)=  results_c_k(ini_k,timestep);
end

Results_end_max(r_simu_2+1)  = max(results_c_k(:,timestep));                     %essential for plotting results
Results_end_mean(r_simu_2+1) = mean(results_c_k(:,timestep));                    %essential for plotting results
...
while count_max <= max_nr_generation-2 && Results_end_mean(r_simu_2+1)< conc_nr && conv_crit < nr_convergenz
    ...
    %population -> probability for parametersets with a high
    %calciumconcentration is higher
    sel_sum_c_k = sum(parent_matrix(1:ini_popsize,4));
    ...
    for mpar_i = 1:sel_2/2                                                              %divided by 2, because there are 2 parents
        ...
        for mpar_j = 1:ini_popsize/nr_parents;  
    ...
    Results_end_max(r_simu_2+2)  = max(results_c_k(:,timestep));                     %essential for plotting results
    Results_end_mean(r_simu_2+2) = mean(results_c_k(:,timestep));                    %essential for plotting results
