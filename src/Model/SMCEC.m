...
%           c_w_i = 1 ./ (p.epsilon_i + p.alpha_i * exp(p.gam_i * cGMP_i)); % NO included - new (but no epsilon_i, alpha_i, gam_i defined here)
     %       c_w_i = 0; % NO excluded
            ...
            %+0.1*J_stretch_i changed to -0.1*J_stretch_i to have correct direction of flux through SAC
            ...
            du(idx.Ca_j, :) = J_IP3_j - J_ER_uptake_j + J_CICR_j - ...
                ...
                J_stretch_j - J_Ca_coup_i;                                      %+J_stretch_j changed to -J_stretch_j to have correct direction of flux through SAC --- why no 0.1?
...
%         end
        ...
        function PLCchange = input_PLC(self, t)   %TimvdBoom
            ...
            PLCchange = p.PLCSwitch * (p.J_PLC_vasomotion - p.J_PLC_steadystate) * ( ...
                1 * tanh((t - p.PLC_t_1) /1) - ...
                1 * tanh((t - p.PLC_t_2) / 1)) + p.J_PLC_steadystate;
        end
...
%             
%             p = self.params;
%             PLCchange = p.PLCSwitch * (p.J_PLC_vasomotion - p.J_PLC_steadystate)*(t / p.PLC_t_2)+ p.J_PLC_steadystate;
%         end        
