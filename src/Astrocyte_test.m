classdef Astrocyte_test < handle
    ...
    methods
        function self = Astrocyte_test(varargin)
...
%            phi_w = p.psi_w * cosh((v_k + v_6) / (2 * p.v_4)); % Ca excluded
        ...
        %% Conservation Equations
            ...
            du(idx.N_K_s, :) = J_K_k - 2 * J_NaK_k - J_NKCC1_k - J_KCC1_k + J_K_NEtoSC .* R_s;
            du(idx.N_Na_s, :) = - du(idx.N_Na_k, :) + J_Na_NEtoSC .* R_s;
%             du(idx.N_K_s, :) = J_K_k - 2 * J_NaK_k - J_NKCC1_k - J_KCC1_k + p.k_C * self.input_K(t);
%             du(idx.N_Na_s, :) = - du(idx.N_Na_k, :) - p.k_C * self.input_K(t);
