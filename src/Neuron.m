classdef Neuron < handle
    ...
    methods
        ...
        function [du, varargout] = rhs(self, t, u, NO_k, R_s, R)
            t = t(:).';
            p = self.params;
            idx = self.index;
        ...
        %% Algebraic expressions       
            ...
            I_Ca        = (-4 * v_sa * p.G_M * p.P_Ca_P_M * (p.Ca_ex / p.M)) / (1 + exp(-80 * (v_sa + 0.02)))...         
                            * (exp(2 * v_sa * p.Farad / (p.R_gas * p.T))) / (1 - exp(2 * v_sa * p.Farad / (p.R_gas * p.T))); %[fA]
            ...
            E_Na_sa     = ph .* log(Na_e ./ Na_sa);
            E_K_sa      = ph .* log(K_e ./ K_sa);
            E_Na_d      = ph .* log(Na_e ./ Na_d);
            E_K_d       = ph .* log(K_e ./ K_d);
            
            %% Ion fluxes
            % Leak fluxes of Na,K,Cl in soma and dendrite using HH
            J_Naleak_sa = gNaleak_sa * (v_sa - E_Na_sa);
            J_Kleak_sa  = gKleak_sa * (v_sa - E_K_sa);
            J_Naleak_d  = gNaleak_d * (v_d - E_Na_d);
            J_Kleak_d   = gKleak_d * (v_d - E_K_d);
            ...
            J_NaP_sa    = (m1.^2 .* h1 .* gNaP_GHk .* F .* v_sa .* (Na_sa - (exp(-v_sa ./ ph) .* Na_e))) ./ (ph .* (1 - exp(-v_sa ./ ph)));
            ...
            J_NaT_sa    = (m8.^2 .* h6 .* gNaT_GHk .* F .* v_sa .* (Na_sa - (exp(-v_sa ./ ph) .* Na_e))) ./ (ph .* (1 - exp(-v_sa ./ ph)));

            % K flux through KDR channel in soma using GHK
            m2alpha     = 0.016 .* ((v_sa + 34.9) ./ (1 - exp(-((0.2 .* v_sa) + 6.98))));
            m2beta      = 0.25 .* exp(-((0.025 .* v_sa) + 1.25));
            J_KDR_sa    =(m2.^2 .* gKDR_GHk * F * v_sa .* (K_sa - (exp(-v_sa ./ ph) .* K_e))) ./ (ph .* (1 - exp(-v_sa ./ ph)));
            ...
            J_KA_sa     = (m3.^2 .* h2 .* gKA_GHk .* F .* v_sa .* (K_sa - (exp(-v_sa ./ ph) .* K_e))) ./ (ph .* (1 - exp(-v_sa ./ ph)));
            ...
            J_NaP_d     = (m4.^2 .* h3 .* gNaP_GHk .* F .* v_d .* (Na_d - (exp(-v_d ./ ph) .* Na_e))) ./ (ph .* (1 - exp(-v_d ./ ph)));
            ...
            J_NMDA_d    = ((m5 .* h4 .* gNMDA_GHk .* F .* v_d .* (Na_d - (exp(-v_d ./ ph) .* Na_e))) ./ (ph .* (1 - exp(-v_d ./ ph)))) ./ (1 + 0.33 * Mg * exp(-(0.07 * v_d + 0.7)));

            % K flux through KDR channel in dendrite using GHK
            m6alpha     = 0.016 .* ((v_d + 34.9) ./ (1 - exp(-((0.2 .* v_d) + 6.98))));
            m6beta      = 0.25 .* exp(-((0.025 .* v_d) + 1.25));
            J_KDR_d     = (m6.^2 .* gKDR_GHk .* F .* v_d .* (K_d - (exp(-v_d ./ ph) .* K_e))) ./ (ph .* (1 - exp(-v_d ./ ph)));
            ...
            J_KA_d      = (m7.^2 .* h5 .* gKA_GHk .* F .* v_d .* (K_d - (exp(-v_d ./ ph) .* K_e))) ./ (ph .* (1 - exp(-v_d ./ ph)));
            
            % flux through the NaK-ATPase pump
            J_pump1_sa      = (1 + (K_init_e ./ K_e)).^(-2) .* (1 + (Na_init_e ./ Na_sa)) .^ (-3);
            J_pump1init_sa  = (1 + (K_init_e ./ K_init_e)).^(-2) .* (1 + (Na_init_e ./ Na_init_e)).^(-3);
            J_pump1_d       = (1 + (K_init_e ./ K_e)).^(-2) .* (1 + (Na_init_d ./ Na_d)).^(-3);
            J_pump1init_d   = (1 + (K_init_e ./ K_init_e)).^(-2) .* (1 + (Na_init_d ./ Na_init_d)).^(-3);
            J_pump2         = 2 .* (1 + O2_0 ./ (((1 - alph) .* O2) + alph .* O2_0)).^(-1);

            %NE(flu.N_sa_pumprate)= -(J_NaP_sa+ J_Naleak_sa+J_KDR_sa+ J_KA_sa+J_Kleak_sa+J_Cl_tot_sa)/(J_pump1_sa.*J_pump2);
            %NE(flu.N_d_pumprate)= -(J_NaP_d+J_Naleak_d+ J_NMDA_d+J_KDR_d+ J_KA_d+J_Kleak_d+ J_NMDA_d+ J_Cl_tot_d)/(J_pump1_d.*J_pump2);
            ...
            J_Cl_tot_sa = gClleak_sa * (v_sa- E_Cl_sa); % leak 
            ...
            J_Cl_tot_d  = gClleak_d * (v_d - E_Cl_d); % leak
            ...
            %% Tissue oxygen 
            P_02            = 2 * (1 + O2_0 ./ ((1 - alph) .* O2 + alph .* O2_0)).^(-1) - (2 * (1 + (O2_0 ./ (alph .* O2_0))).^(-1)) ./ (2 .* (1 + O2_0 ./ ((1 - alph) .* O2_0 + alph .* O2_0)).^(-1)) - (2 * (1 + (O2_0 ./ (alph .* O2_0))).^(-1));  % P(02)
            CBF             = CBF_init .* (R.^4 ./ LU_R_init^4);
            J_O2_vascular   = CBF .* ((O2_b - O2) ./ (O2_b - O2_0));
            J_O2_background = CBF_init .* P_02 .* (1 - gamm);
            J_O2_pump       = CBF_init .* P_02 .* gamm .* ((J_pump1_sa + J_pump1_d) ./ (J_pump1init_sa + J_pump1init_d));
        ...
        %% Conservation equations
            ...
            du(idx.v_sa, :)     = 1/Cm .* ( -J_tot_sa + 1 ./ (2 * Ra * dhod.^2) * (v_d - v_sa) + 0.008 * rectpuls(time-2,0.8) );
            du(idx.v_d, :)      = 1/Cm .* (-J_tot_d + 1 ./ (2 * Ra * dhod.^2) * (v_sa - v_d));

            %change in concentration of Na,K,Cl in the soma
            du(idx.Na_sa, :)    = -As ./ (F .* Vs) .* J_Na_tot_sa + D_Na .* (Vd + Vs) ./ (2 * dhod.^2 .* Vs) .* (Na_d - Na_sa);
            du(idx.K_sa, :)     = -As ./ (F .* Vs) .* J_K_tot_sa + D_K .* (Vd + Vs) ./ (2 * dhod.^2 .* Vs) .* (K_d - K_sa);
            du(idx.Cl_sa, :)    = -As ./ (F .* Vs) .* J_Cl_tot_sa + D_Cl .* (Vd + Vs) ./ (2 * dhod.^2 .* Vs) .* (Cl_d - Cl_sa);

            %change in concentration of Na,K,Cl in the dendrite
            du(idx.Na_d, :)     = -Ad ./ (F .* Vd) .* J_Na_tot_d + D_Na .* (Vs + Vd) ./ (2 * dhod.^2 .* Vd) .* (Na_sa - Na_d);
            du(idx.K_d, :)      = -Ad ./ (F .* Vd) .* J_K_tot_d + D_K .* (Vs + Vd) ./ (2 * dhod.^2 .* Vd) .* (K_sa - K_d);
            du(idx.Cl_d, :)     = -Ad./(F .* Vd) .* J_Cl_tot_d + D_Cl .* (Vs + Vd) ./ (2 * dhod.^2 .* Vd) .* (Cl_sa - Cl_d);
            
            % change in concentration of Na,K,Cl in the extracellular space 
            du(idx.Na_e, :)     = 1./F * (((As .* J_Na_tot_sa) ./ (Vs .* fe)) + ((Ad .* J_Na_tot_d) ./ (Vd .* fe)));
            du(idx.K_e, :)      = 1./F * (((As .* J_K_tot_sa) ./ (Vs .* fe))  + ((Ad .* J_K_tot_d) ./ (Vd .* fe))) - du(idx.Buff);
            du(idx.Cl_e, :)     = 1./F * (((As .* J_Cl_tot_sa) ./ (Vs .* fe)) + ((Ad .* J_Cl_tot_d) ./ (Vd .* fe)));
            
            % change in potassium buffer for K+ in the extracellular space
            du(idx.Buff, :)     = Mu .* K_e .* (B0 - Buff) ./ (1 + exp(-((K_e - 15) ./ 1.09))) - (Mu .* Buff);
       ...
       function [J_K_SC, J_Na_SC, NO_n] = shared(self, t, u)    %shared variable
            t = t(:).';
            p = self.params;
            idx = self.index;
...
function params = parse_inputs(varargin)
    ...
    parser.addParameter('F', 96.485);           % Faraday constant [C / mmol]
    ...
    parser.addParameter('gClleak_sa', 10*gNaleak_sa);
    parser.addParameter('gNaleak_d', 4.135e-5);   
    parser.addParameter('gKleak_d', 1.4148e-4); 
    parser.addParameter('gClleak_d', 10*gNaleak_d);   
