classdef Astrocyte_1 < handle
    ...
    methods
        function self = Astrocyte_1(varargin)
        ...
        function [du, varargout] = rhs(self, t, u, J_KIR_i_1)
            ...
            N_Na_k_1 = u(idx.N_Na_k_1, :);
            N_K_k_1 = u(idx.N_K_k_1, :);
            N_Cl_k_1 = u(idx.N_Cl_k_1, :);
            N_HCO3_k_1 = u(idx.N_HCO3_k_1, :);
            
            N_Na_s_1 = u(idx.N_Na_s_1, :);
            N_K_s_1 = u(idx.N_K_s_1, :);
            N_HCO3_s_1 = u(idx.N_HCO3_s_1, :);
            ...
            N_Cl_s_1 = N_Na_s_1 + N_K_s_1 - N_HCO3_s_1;
            ...
            HCO3_s_1 = N_HCO3_s_1 ./ R_s_1;
            Na_k_1 = N_Na_k_1 ./ R_k_1;
            K_k_1 = N_K_k_1 ./ R_k_1;
            Cl_k_1 = N_Cl_k_1 ./ R_k_1;
            HCO3_k_1 = N_HCO3_k_1 ./ R_k_1;
            
            %% Astrocyte 
            % Volume-surface Ratio; Scaling ODE
            du(idx.R_k_1, :) = p.L_p * ( ...
                Na_k_1 + K_k_1 + Cl_k_1 + HCO3_k_1 - ...
                Na_s_1 - Cl_s_1 - K_s_1 - HCO3_s_1 + p.X_k ./ R_k_1);
            
            % Nernst potentials
            E_K_k_1 = p.R_g * p.T / (p.z_K * p.F) * log(K_s_1 ./ K_k_1);
            E_Na_k_1 = p.R_g * p.T / (p.z_Na * p.F) * log(Na_s_1 ./ Na_k_1);
            E_Cl_k_1 = p.R_g * p.T / (p.z_Cl * p.F) * log(Cl_s_1 ./ Cl_k_1);
            E_NBC_k_1 = p.R_g * p.T / (p.z_NBC * p.F) * ...
                log((Na_s_1 .* HCO3_s_1.^2) ./ (Na_k_1 .* HCO3_k_1.^2));
            E_BK_k_1 = p.R_g * p.T / (p.z_K * p.F) * log(K_p_1 ./ K_k_1);
            
            % Flux through the Sodium Potassium pump
            J_NaK_k_1 = p.J_NaK_max * Na_k_1.^1.5 ./ ...
                (Na_k_1.^1.5 + p.K_Na_k^1.5) .* ...
                K_s_1 ./ (K_s_1 + p.K_K_s);
            
            % Membrane voltage
            v_k_1 = (p.g_Na_k * E_Na_k_1 + p.g_K_k * E_K_k_1 + ...
                p.g_Cl_k * E_Cl_k_1 + p.g_NBC_k * E_NBC_k_1 + ...
                p.g_BK_k * w_k_1 .* E_BK_k_1 - ...
                J_NaK_k_1 * p.F / p.C_correction) ./ ...
                (p.g_Na_k + p.g_K_k + p.g_Cl_k + p.g_NBC_k + ...
                p.g_BK_k * w_k_1);
            
            % Fluxes
            J_BK_k_1 = p.g_BK_k / p.F * w_k_1 .* ...
                (v_k_1 - E_BK_k_1) * p.C_correction;
            J_K_k_1 = p.g_K_k / p.F * (v_k_1 - E_K_k_1) * p.C_correction;
            
            J_Na_k_1 = p.g_Na_k / p.F * (v_k_1 - E_Na_k_1) * p.C_correction;
            J_NBC_k_1 = p.g_NBC_k / p.F * (v_k_1 - E_NBC_k_1) * p.C_correction;
            J_KCC1_k_1 = self.flux_ft_1(t) .* p.g_KCC1_k / p.F * p.R_g * ...
                p.T / p.F .* ...
                log((K_s_1 .* Cl_s_1) ./ (K_k_1 .* Cl_k_1)) * p.C_correction;
            J_NKCC1_k_1 = self.flux_ft_1(t) * p.g_NKCC1_k / p.F * p.R_g * ...
                p.T / p.F .* log((Na_s_1 .* K_s_1 .* Cl_s_1.^2) ./ ...
                (Na_k_1 .* K_k_1 .* Cl_k_1.^2)) * p.C_correction;
            
            
            % Aditional equations
            w_inf_1 = 0.5 * ...
                (1 + tanh((v_k_1 + p.v_6) / p.v_4));
            
            phi_w_1 = p.psi_w * cosh((v_k_1 + p.v_6) / (2*p.v_4));
            
            %% Conservation Equations
            % Differential Equations in the Astrocyte
            du(idx.N_K_k_1, :) = -J_K_k_1 + 2*J_NaK_k_1 + J_NKCC1_k_1 + ...
                J_KCC1_k_1 - J_BK_k_1;
            du(idx.N_Na_k_1, :) = -J_Na_k_1 - 3*J_NaK_k_1 + J_NKCC1_k_1 + J_NBC_k_1;
            
            du(idx.N_HCO3_k_1, :) = 2*J_NBC_k_1;
            du(idx.N_Cl_k_1, :) = du(idx.N_Na_k_1, :) + du(idx.N_K_k_1, :) - ...
                du(idx.N_HCO3_k_1, :);
           
            du(idx.w_k_1, :) = phi_w_1 .* (w_inf_1 - w_k_1);
            
            % Differential Equations in the Perivascular space
            du(idx.K_p_1, :) = J_BK_k_1 ./ (R_k_1 * p.VR_pa) + J_KIR_i_1 ./ ...
                p.VR_ps - p.R_decay*(K_p_1 - p.K_p_min);
            
            % Differential Equations in the Synaptic Cleft
            du(idx.N_K_s_1, :) = p.k_C * self.input_f_1(t) - ...
                du(idx.N_K_k_1, :) - J_BK_k_1;
            du(idx.N_Na_s_1, :) = -p.k_C * self.input_f_1(t) - ...
                du(idx.N_Na_k_1, :);
            du(idx.N_HCO3_s_1, :) = -du(idx.N_HCO3_k_1, :);
            du = bsxfun(@times, self.enabled, du);
            if nargout == 2
               Uout = zeros(self.n_out, size(u, 2));
               Uout(self.idx_out.ft_1, :) = self.input_f_1(t);
               Uout(self.idx_out.v_k_1, :) = v_k_1;
               Uout(self.idx_out.K_s_1, :) = K_s_1;
               Uout(self.idx_out.K_p_1, :) = K_p_1;
               Uout(self.idx_out.J_BK_k_1, :) = J_BK_k_1;
               Uout(self.idx_out.w_inf_1, :) = w_inf_1;
               Uout(self.idx_out.phi_w_1, :) = phi_w_1;
              
               varargout = {Uout};
            end
        end        
        function K_p_1 = shared(self, ~, u)
            K_p_1 = u(self.index.K_p_1, :);
        end
        function f_1 = input_f_1(self, t)
            ...
            f_1(p.t_2 <= t & t <= p.t_3) = -p.F_input;
        end
        function out_1 = flux_ft_1(self, t)
...
u0(idx.R_k_1) = 0.061e-6;
u0(idx.N_Na_k_1) = 0.99796e-3;
u0(idx.N_K_k_1) = 5.52782e-3;
u0(idx.N_HCO3_k_1) = 0.58804e-3;
u0(idx.N_Cl_k_1) = 0.32879e-3;
u0(idx.N_Na_s_1) = 4.301041e-3;
u0(idx.N_K_s_1) = 0.0807e-3;
u0(idx.N_HCO3_s_1) = 0.432552e-3;
u0(idx.K_p_1) = 3e3;
u0(idx.w_k_1) = 0.1815e-3;
end
