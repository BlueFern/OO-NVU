...
for i=[1:length(lijst)]
    ...
    kleur=['k','b','r','g'];
    hold on
    plot(simulatie.J_PLC_value(nummer,:),simulatie.wssmaximum(nummer,:), 'color',kleur(i),'LineWidth',2.5)
    plot(simulatie.J_PLC_value(nummer,:),simulatie.wssminimum(nummer,:), 'color',kleur(i),'LineWidth',2.5)
 %   plot(simulatie.J_PLC_value(nummer,:),simulatie.NOmean(nummer,:), 'color',kleur(i),'LineWidth',2.5)
    myfit=polyfit(simulatie.J_PLC_value(nummer,:),simulatie.wssmean(nummer,:),4)
    ...
    ylabel('Wall shear stress [Pa]', 'FontSize', 26)
    box on
