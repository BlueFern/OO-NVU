...
for i=[1:length(lijst)]
    ...
    kleur=['k','b','r','g'];
    hold on
    plot(simulatie.J_PLC_value(nummer,:),simulatie.radiusmaximum(nummer,:)*1e6, 'color',kleur(i),'LineWidth',2.5)
    plot(simulatie.J_PLC_value(nummer,:),simulatie.radiusminimum(nummer,:)*1e6, 'color',kleur(i),'LineWidth',2.5)
    myfit=polyfit(simulatie.J_PLC_value(nummer,:),simulatie.radiusmean(nummer,:)*1e6,4);
    ...
    ylabel('Radius[\muM]', 'FontSize', 26)
    box on
  %  title('radius of the vessel with different pressure')    
