...
for i=[1:length(lijst)]
    ...
    kleur=['k','b','r','g'];
    hold on
    plot(simulatie.J_PLC_value(nummer,:),simulatie.vimaximum(nummer,:), 'color',kleur(i),'LineWidth',2.5)
    plot(simulatie.J_PLC_value(nummer,:),simulatie.viminimum(nummer,:), 'color',kleur(i),'LineWidth',2.5)
%    myfit=polyfit(simulatie.J_PLC_value(nummer,:),simulatie.radiusmean(nummer,:)*1e6,4)
 ...
 %   plot(X22(i,:),Y2, '--', 'color',kleur(i),'LineWidth',2.5)
    ...
    ylabel('Membrame potential [mV]', 'FontSize', 26)
    box on
