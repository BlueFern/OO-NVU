...
for i=[1:length(lijst)]
    ...
    kleur=['k','b','r','g'];
    hold on
    plot(simulatie.J_PLC_value(nummer,:),simulatie.Pressure(nummer,:).*(simulatie.radiusmaximum(nummer,:))./(simulatie.radiusmaximum(nummer,:)*0.1), 'color',kleur(i),'LineWidth',2.5)
    plot(simulatie.J_PLC_value(nummer,:),simulatie.Pressure(nummer,:).*(simulatie.radiusmaximum(nummer,:))./(simulatie.radiusminimum(nummer,:)*0.1), 'color',kleur(i),'LineWidth',2.5)
 %   plot(simulatie.J_PLC_value(nummer,:),simulatie.NOmean(nummer,:), 'color',kleur(i),'LineWidth',2.5)
   ...
   % plot(X22(i,:),Y2, '--', 'color',kleur(i),'LineWidth',2.5)
    ...
    ylabel('Wall thickness [\mum]', 'FontSize', 26)
    box on
